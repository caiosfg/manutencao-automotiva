...
class CarrosController extends Controller
{
    ...
    {
        $validator =  ValidationCar::make(
            $request->all(),
            ValidationCar::RULE_CAR
        );

        if ($validator->fails()) {
            return response()->json($validator->error(), Response::HTTP_BAD_REQUEST);
        } else {
            try {
                $carro = $this->model->find($id)
                ->update($request->all());
                ...
                return response()->json(['error' => 'Erro de conexão com Banco'], Response::HTTP_INTERNAL_SERVER_ERROR);
            }
        }
